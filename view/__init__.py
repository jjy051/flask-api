...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if access_token is not None:
            ...
            if payload is None: return Response(status=401, response='invalid token')

            user_id = payload['user_id']
            g.user_id = user_id
        else:
            return Response(status = 401, response='header parsing error')
...
def create_endpoints(app, services):
    ...
    def upload_profile_picture():
        ...
        ## to defense file name hacking (SQL Injection Attack)
